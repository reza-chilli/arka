...
module.exports = {
  async projectDataTableRender(req, res) {
    ...
    const rawData = fs.readFileSync(
      path.join(__dirname, "../views/projects/project.json")
    );
    const projectDataTableContent = JSON.parse(rawData);
    const data = {
      error: req.flash("error"),
      success: req.flash("success"),
      generalContent,
      content: projectDataTableContent[user.settings.language || "english"],
    };
